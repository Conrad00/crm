...
//		$CI =& get_instance();
	 ...
	 */	
	function go_upload($field = 'userfile',$category=1) {
        $CI =& get_instance(); 
...
//            echo "$fname\n\n<br><br>\n\n";
			...
			$CI->upload->file_raw  = str_replace($ext," ",$_FILES[$field]['name'][$i]);
...
// Add error display for individual files        
        if ($error_upload) {
        	...
        	$this->thumb($CI->upload->file_name,$CI->upload->upload_path,$CI->upload->data(),$category);
            return $file_list;;
        }    
    }
	...
	function thumb($file_name,$path,$raw,$category){
				 $CI =& get_instance();  
				//print_r($data);
			...
			$tumb=$raw['raw_name']."_thumb".$raw['file_ext'];
				$sql_data = array(
				'filename'			=>	$file_name ,
				'submitdate'		=>	mktime(),
				'is_active'			=>	1 ,
